...
def main():
    ...
    if ds == 'CiteSeer':
        ...
        data = dataset[0]
    elif ds == 'barabasi_albert':
        data = torch.load('barabasi_albert_graph_42.pt')
    elif ds == "erdos_renyi":
        data = torch.load('erdos_renyi_graph_42.pt')
    elif ds.startswith("sbm"):
        match = re.fullmatch(r"sbm_(\d+\.\d+)_(\d+\.\d+)_(\d+)", ds)
        if match:
            m1 = float(match.group(1))
            m2 = float(match.group(2))
            m3 = int(match.group(3))
            data = torch.load(f'sbm_{m1}_{m2}_{m3}.pt')
    elif ds.startswith("barbell"):
        match = re.fullmatch(r"barbell_(\d+)_(\d+)_(\w+)", ds)
        if match:
            m1 = int(match.group(1))
            m2 = int(match.group(2))
            mode = match.group(3)
            data = torch.load(f'barbell_{m1}_{m2}_{mode}.pt')
    else:
    ...
    og_data = T.ToUndirected()(data).to(device)
