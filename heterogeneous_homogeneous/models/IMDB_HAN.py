...
def evaluate_node_classification(model, data, mask):
    ...
    with torch.no_grad():
        ...
        correct = (pred[mask] == data['movie'].y[mask].to(pred.device)).sum()
        acc = correct.float() / mask.sum()
    return acc.item()


def split_train_edges(data, split_ratio):
    total_edges = data['movie', 'train_is', 'label'].edge_index.size(1)
    ...
    msg_edge_index = data['movie', 'train_is', 'label'].edge_index[:, msg_edges]
    sup_edge_index = data['movie', 'train_is', 'label'].edge_index[:, sup_edges]
    return msg_edge_index, sup_edge_index

def my_negative_sampling(pos_edge_index, num_movie_nodes, num_classes, device):
    neg_edge_index = torch.zeros((2, num_movie_nodes * (num_classes - 1)), device=device)
    neg_edge_index[0, :] = pos_edge_index[0].repeat(num_classes - 1)
    labels = torch.arange(num_classes, device=device)
    
    for i in range(num_movie_nodes):
...
def prepare_data(data, device, num_classes, msg_split_ratio):
    ...
    unique_classes = data['movie'].y.unique()

    data['label'].x = torch.eye(len(unique_classes), dtype=torch.float32, device=device)

    train_mask_indices = torch.nonzero(data['movie'].train_mask, as_tuple=False).flatten()
    ...
    train_label_edges[1] = data['movie'].y[train_mask_indices]
    data['movie', 'train_is', 'label'].edge_index = train_label_edges

    val_mask_indices = torch.nonzero(data['movie'].val_mask, as_tuple=False).flatten()
    val_label_edges = torch.zeros((2, val_mask_indices.size(0)), dtype=torch.int64, device=device)
    val_label_edges[0] = val_mask_indices
    val_label_edges[1] = data['movie'].y[val_mask_indices]
    data['movie', 'val_is', 'label'].edge_index = val_label_edges

    test_mask_indices = torch.nonzero(data['movie'].test_mask, as_tuple=False).flatten()
    ...
    test_label_edges[1] = data['movie'].y[test_mask_indices]
    data['movie', 'test_is', 'label'].edge_index = test_label_edges


    msg_edge_index, sup_edge_index = split_train_edges(data, msg_split_ratio)

    data['movie', 'train_is', 'label'].edge_index = msg_edge_index
    data['movie', 'train_is', 'label'].edge_label_index = sup_edge_index
    data['movie', 'train_is', 'label'].edge_label = torch.tensor(np.ones((sup_edge_index.size(1))))

    data['movie', 'val_is', 'label'].edge_label_index = data['movie', 'val_is', 'label'].edge_index
    data['movie', 'val_is', 'label'].edge_index = torch.cat((msg_edge_index, sup_edge_index), 1)
    data['movie', 'val_is', 'label'].edge_label = torch.tensor(np.ones((data['movie', 'val_is', 'label'].edge_label_index.size(1))))

    data['movie', 'test_is', 'label'].edge_label_index = data['movie', 'test_is', 'label'].edge_index
    data['movie', 'test_is', 'label'].edge_index = torch.cat(
        (msg_edge_index, sup_edge_index, data['movie', 'val_is', 'label'].edge_label_index), 1
    )
    data['movie', 'test_is', 'label'].edge_label = torch.tensor(
        np.ones((data['movie', 'test_is', 'label'].edge_label_index.size(1)))
    )
    ...
    data['movie', 'train_is', 'label'].neg_edge_index = my_negative_sampling(data['movie', 'train_is', 'label'].edge_label_index, data['movie', 'train_is', 'label'].edge_label_index.size(1), num_classes, device)
...
def create_metapaths_lp(data):
    metapaths = [
        [('movie', 'to', 'actor'), ('actor', 'to', 'movie')],
        [('movie', 'to', 'director'), ('director', 'to', 'movie')],
        [('movie', 'train_is', 'label'),
         ('label', 'rev_train_is', 'movie'),
         ('movie', 'to', 'actor'),
         ('actor', 'to', 'movie')],#MLMAM
        [('movie', 'train_is', 'label'),
         ('label', 'rev_train_is', 'movie'),
         ('movie', 'to', 'director'),
         ('director', 'to', 'movie')],#MLMDM
         [('movie', 'val_is', 'label'),
         ('label', 'rev_val_is', 'movie'),
         ('movie', 'to', 'actor'),
         ('actor', 'to', 'movie')],#MLMAM
        [('movie', 'val_is', 'label'),
         ('label', 'rev_val_is', 'movie'),
    ...
    lp_data['movie', 'metapath_2', 'movie'].edge_index = sample_edges(
        lp_data['movie', 'metapath_2', 'movie'].edge_index, 30000)
    lp_data['movie', 'metapath_3', 'movie'].edge_index = sample_edges(
        lp_data['movie', 'metapath_3', 'movie'].edge_index, 30000)
    lp_data['movie', 'metapath_4', 'movie'].edge_index = sample_edges(
        lp_data['movie', 'metapath_4', 'movie'].edge_index, 30000)
    lp_data['movie', 'metapath_5', 'movie'].edge_index = sample_edges(
        lp_data['movie', 'metapath_5', 'movie'].edge_index, 30000)
    lp_data['movie', 'metapath_6', 'movie'].edge_index = sample_edges(
        lp_data['movie', 'metapath_6', 'movie'].edge_index, 30000)
    lp_data['movie', 'metapath_7', 'movie'].edge_index = sample_edges(
        lp_data['movie', 'metapath_7', 'movie'].edge_index, 30000)
...
class EdgeDecoder(torch.nn.Module):
    ...
    def forward(self, z_movie, z_label):
        x = z_movie * z_label
...
class HAN_LP(nn.Module):
    ...
    def decode(self, z_dict, edge_label_index):
        ...
        out = self.decoder(z_movie[src], z_label[dst])
        return out
...
def train_link_prediction(model, data, optimizer, criterion, train_edge_index_dict):
    ...
    pos_edge_index = data['movie', 'train_is', 'label'].edge_label_index
    pos_pred = model.decode(z_dict, pos_edge_index)
    neg_edge_index = data['movie', 'train_is', 'label'].neg_edge_index
...
def evaluate_link_prediction(model, data, mask, edge_dict):
    ...
    with torch.no_grad():
        ...
        movie_embeddings = z_dict['movie'][mask]
        label_embeddings = z_dict['label']
        z_movie = movie_embeddings.unsqueeze(1)
        z_label = label_embeddings.unsqueeze(0)
        x = z_movie * z_label
        x = x.view(-1, x.size(-1))
        pred_scores = model.decoder.lin(x)
        pred_scores = pred_scores.view(z_movie.size(0), z_label.size(1))
        predicted_labels = pred_scores.argmax(dim=1)
        true_labels = data['movie'].y[mask]
...
def save_run(test_acc_NC, test_acc_LP, hidden_channels, embedding_size, num_epochs, lr_NC, lr_LP, num_seeds, msg_split_ratio):
    ...
    save_path = os.path.join("runs", "HAN_IMDB", run_name)
...
def main():
    ...
    for seed in seed_list:
        ...
        num_classes = data['movie'].y.max().item() + 1

        nc_data = create_metapaths_nc(data)

        in_channels_dict_NC = {
            'movie': nc_data['movie'].num_features,
            'actor': nc_data['actor'].num_features,
            'director': nc_data['director'].num_features
        }
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            train_nc = evaluate_node_classification(model_NC, nc_data, nc_data['movie'].train_mask)
            val_nc = evaluate_node_classification(model_NC, nc_data, nc_data['movie'].val_mask)
            test_nc = evaluate_node_classification(model_NC, nc_data, nc_data['movie'].test_mask)
            if print_epochs:
        ...
        # uncomment below to use metapaths involving label nodes as well
        train_edge_index_dict = {
          ('movie','MAM','movie'): lp_data['movie','metapath_0','movie'].edge_index,
          ('movie','MDM','movie'): lp_data['movie','metapath_1','movie'].edge_index,
          #('movie','MLMAM','movie'): lp_data['movie','metapath_2','movie'].edge_index,
          #('movie','MLMDM','movie'): lp_data['movie','metapath_3','movie'].edge_index,
          ('movie','is','label'): lp_data['movie','train_is','label'].edge_index,
          ('label','rev_is','movie'): lp_data['label','rev_train_is','movie'].edge_index,
        }

        val_edge_index_dict = {
          ('movie','MAM','movie'): lp_data['movie','metapath_0','movie'].edge_index,
          ('movie','MDM','movie'): lp_data['movie','metapath_1','movie'].edge_index,
          #('movie','MLMAM','movie'): lp_data['movie','metapath_4','movie'].edge_index,
          #('movie','MLMDM','movie'): lp_data['movie','metapath_5','movie'].edge_index,
          ('movie','is','label'): lp_data['movie','val_is','label'].edge_index,
          ('label','rev_is','movie'): lp_data['label','rev_val_is','movie'].edge_index,
        }

        test_edge_index_dict = {
          ('movie','MAM','movie'): lp_data['movie','metapath_0','movie'].edge_index,
          ('movie','MDM','movie'): lp_data['movie','metapath_1','movie'].edge_index,
          #('movie','MLMAM','movie'): lp_data['movie','metapath_6','movie'].edge_index,
          #('movie','MLMDM','movie'): lp_data['movie','metapath_7','movie'].edge_index,
          ('movie','is','label'): lp_data['movie','test_is','label'].edge_index,
          ('label','rev_is','movie'): lp_data['label','rev_test_is','movie'].edge_index,
        }

        in_channels_dict_LP = {
            'movie': lp_data['movie'].num_features,
            'actor': lp_data['actor'].num_features,
            'director': lp_data['director'].num_features,
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            train_lp = evaluate_link_prediction(model_LP, lp_data, lp_data['movie'].train_mask, train_edge_index_dict)
            val_lp = evaluate_link_prediction(model_LP, lp_data, lp_data['movie'].val_mask, val_edge_index_dict)
            test_lp = evaluate_link_prediction(model_LP, lp_data, lp_data['movie'].test_mask, test_edge_index_dict)
            if print_epochs:
                print(f'Epoch: {epoch:03d}, '
                      f'Loss: {loss_lp:.4f}, '
