...
def train_node_classification(model, data, optimizer, criterion):
    ...
    loss = criterion(out[data['author'].train_mask], data['author'].y[data['author'].train_mask])
    loss.backward()
    optimizer.step()
    return loss.item()
...
def evaluate_node_classification(model, data, mask):
    ...
    with torch.no_grad():
        ...
        correct = (pred[mask] == data['author'].y[mask].to(pred.device)).sum()
        acc = correct.float() / mask.sum()
    return acc.item()


def split_train_edges(data, split_ratio):
    total_edges = data['author', 'train_is', 'label'].edge_index.size(1)
    ...
    msg_edge_index = data['author', 'train_is', 'label'].edge_index[:, msg_edges]
    sup_edge_index = data['author', 'train_is', 'label'].edge_index[:, sup_edges]
    return msg_edge_index, sup_edge_index

def my_negative_sampling(pos_edge_index, num_author_nodes, num_classes, device):
    neg_edge_index = torch.zeros((2, num_author_nodes * (num_classes - 1)), device=device)
    neg_edge_index[0, :] = pos_edge_index[0].repeat(num_classes - 1)
    labels = torch.arange(num_classes, device=device)
    
    for i in range(num_author_nodes):
...
def prepare_data(data, device, num_classes, msg_split_ratio):
    ...
    unique_classes = data['author'].y.unique()

    data['label'].x = torch.eye(len(unique_classes), dtype=torch.float32, device=device)

    train_mask_indices = torch.nonzero(data['author'].train_mask, as_tuple=False).flatten()
    ...
    train_label_edges[1] = data['author'].y[train_mask_indices]
    data['author', 'train_is', 'label'].edge_index = train_label_edges

    val_mask_indices = torch.nonzero(data['author'].val_mask, as_tuple=False).flatten()
    val_label_edges = torch.zeros((2, val_mask_indices.size(0)), dtype=torch.int64, device=device)
    val_label_edges[0] = val_mask_indices
    val_label_edges[1] = data['author'].y[val_mask_indices]
    data['author', 'val_is', 'label'].edge_index = val_label_edges

    test_mask_indices = torch.nonzero(data['author'].test_mask, as_tuple=False).flatten()
    ...
    test_label_edges[1] = data['author'].y[test_mask_indices]
    data['author', 'test_is', 'label'].edge_index = test_label_edges


    msg_edge_index, sup_edge_index = split_train_edges(data, msg_split_ratio)

    data['author', 'train_is', 'label'].edge_index = msg_edge_index
    data['author', 'train_is', 'label'].edge_label_index = sup_edge_index
    data['author', 'train_is', 'label'].edge_label = torch.tensor(np.ones((sup_edge_index.size(1))))

    data['author', 'val_is', 'label'].edge_label_index = data['author', 'val_is', 'label'].edge_index
    data['author', 'val_is', 'label'].edge_index = torch.cat((msg_edge_index, sup_edge_index), 1)
    data['author', 'val_is', 'label'].edge_label = torch.tensor(np.ones((data['author', 'val_is', 'label'].edge_label_index.size(1))))

    data['author', 'test_is', 'label'].edge_label_index = data['author', 'test_is', 'label'].edge_index
    data['author', 'test_is', 'label'].edge_index = torch.cat(
        (msg_edge_index, sup_edge_index, data['author', 'val_is', 'label'].edge_label_index), 1
    )
    data['author', 'test_is', 'label'].edge_label = torch.tensor(
        np.ones((data['author', 'test_is', 'label'].edge_label_index.size(1)))
    )
    ...
    data['author', 'train_is', 'label'].neg_edge_index = my_negative_sampling(data['author', 'train_is', 'label'].edge_label_index, data['author', 'train_is', 'label'].edge_label_index.size(1), num_classes, device)
...
def create_metapaths_nc(data):
    metapaths = [
        ...
        [('author','to','paper'), ('paper','to','term'),
        ('term','to','paper'),   ('paper','to','author')],#APTPA
        [('author','to','paper'), ('paper','to','conference'),
        ('conference','to','paper'), ('paper','to','author')],#APCPA
    ]
    ...
    nc_data['author', 'metapath_0', 'author'].edge_index = sample_edges(
        nc_data['author', 'metapath_0', 'author'].edge_index, 300000)
    nc_data['author', 'metapath_1', 'author'].edge_index = sample_edges(
        nc_data['author', 'metapath_1', 'author'].edge_index, 300000)
    nc_data['author', 'metapath_2', 'author'].edge_index = sample_edges(
        nc_data['author', 'metapath_2', 'author'].edge_index, 300000)
...
def create_metapaths_lp(data):
    ...
    lp_data['author', 'metapath_0', 'author'].edge_index = sample_edges(
        lp_data['author', 'metapath_0', 'author'].edge_index, 300000)
    lp_data['author', 'metapath_1', 'author'].edge_index = sample_edges(
        lp_data['author', 'metapath_1', 'author'].edge_index, 300000)
    lp_data['author', 'metapath_2', 'author'].edge_index = sample_edges(
        lp_data['author', 'metapath_2', 'author'].edge_index, 300000)
...
class EdgeDecoder(torch.nn.Module):
    ...
    def forward(self, z_author, z_label):
        x = z_author * z_label
...
class HAN_LP(nn.Module):
    ...
    def forward(self, x_dict, edge_index_dict):
        ...
        h['author'] = self.lin(h['author'])
        h['label'] = self.lin(h['label'])
        return h
    
    def decode(self, z_dict, edge_label_index):
        z_author = z_dict['author']
        z_label = z_dict['label']
        src, dst = edge_label_index
        out = self.decoder(z_author[src], z_label[dst])
        return out
...
def train_link_prediction(model, data, optimizer, criterion, train_edge_index_dict):
    ...
    pos_edge_index = data['author', 'train_is', 'label'].edge_label_index
    pos_pred = model.decode(z_dict, pos_edge_index)
    neg_edge_index = data['author', 'train_is', 'label'].neg_edge_index
...
def evaluate_link_prediction(model, data, mask, edge_dict):
    ...
    with torch.no_grad():
        ...
        author_embeddings = z_dict['author'][mask]
        label_embeddings = z_dict['label']
        z_author = author_embeddings.unsqueeze(1)
        z_label = label_embeddings.unsqueeze(0)
        x = z_author * z_label
        x = x.view(-1, x.size(-1))
        pred_scores = model.decoder.lin(x)
        pred_scores = pred_scores.view(z_author.size(0), z_label.size(1))
        predicted_labels = pred_scores.argmax(dim=1)
        true_labels = data['author'].y[mask]
...
def save_run(test_acc_NC, test_acc_LP, hidden_channels, embedding_size, num_epochs, lr_NC, lr_LP, num_seeds, msg_split_ratio):
    ...
    save_path = os.path.join("runs", "HAN_DBLP", run_name)
...
def main():
    ...
    for seed in seed_list:
        ...
        in_channels_dict_NC = {
            'author': nc_data['author'].num_features,
        }
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            train_nc = evaluate_node_classification(model_NC, nc_data, nc_data['author'].train_mask)
            val_nc = evaluate_node_classification(model_NC, nc_data, nc_data['author'].val_mask)
            test_nc = evaluate_node_classification(model_NC, nc_data, nc_data['author'].test_mask)
            if print_epochs:
        ...
        train_edge_index_dict = {
          ('author','APA','author'): lp_data['author','metapath_0','author'].edge_index,
          ('author','APTPA','author'): lp_data['author','metapath_1','author'].edge_index,
          ('author','APCPA','author'): lp_data['author','metapath_2','author'].edge_index,
          ('author','is','label'): lp_data['author','train_is','label'].edge_index,
          ('label','rev_is','author'): lp_data['label','rev_train_is','author'].edge_index,
        }
        ...
        val_edge_index_dict = {
          ...
          ('author','APTPA','author'): lp_data['author','metapath_1','author'].edge_index,
          ('author','APCPA','author'): lp_data['author','metapath_2','author'].edge_index,
          ('author','is','label'): lp_data['author','val_is','label'].edge_index,
          ('label','rev_is','author'): lp_data['label','rev_val_is','author'].edge_index,
        }
        ...
        test_edge_index_dict = {
          ...
          ('author','APTPA','author'): lp_data['author','metapath_1','author'].edge_index,
          ('author','APCPA','author'): lp_data['author','metapath_2','author'].edge_index,
          ('author','is','label'): lp_data['author','test_is','label'].edge_index,
          ('label','rev_is','author'): lp_data['label','rev_test_is','author'].edge_index,
        }
        ...
        in_channels_dict_LP = {
            ...
            'conference': lp_data['conference'].num_features,
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            train_lp = evaluate_link_prediction(model_LP, lp_data, lp_data['author'].train_mask, train_edge_index_dict)
            val_lp = evaluate_link_prediction(model_LP, lp_data, lp_data['author'].val_mask, val_edge_index_dict)
            test_lp = evaluate_link_prediction(model_LP, lp_data, lp_data['author'].test_mask, test_edge_index_dict)
            if print_epochs:
                print(f'Epoch: {epoch:03d}, '
                      f'Loss: {loss_lp:.4f}, '
