...
from torch_geometric.nn import GINConv, to_hetero, Linear
import time
import json
...
class GIN(torch.nn.Module):
    ...
    def __init__(self, hidden_channels: int, two_layers: bool):
        super().__init__()
        ...
        self.conv1 = GINConv(mlp(hidden_channels))
        if two_layers:
            self.conv2 = GINConv(mlp(hidden_channels))
        else:
            self.conv2 = GINConv(mlp(hidden_channels))
            self.conv3 = GINConv(mlp(hidden_channels))
...
class GIN_NC(torch.nn.Module):
    def __init__(self, hidden_channels, embedding_size, num_classes, two_layers):
        super().__init__()
        self.encoder = GIN(hidden_channels, two_layers)

        self.project = (torch.nn.Identity() if embedding_size == hidden_channels
                        else torch.nn.Linear(hidden_channels, embedding_size))

        self.decoder = torch.nn.Linear(embedding_size, num_classes)

    def forward(self, x, edge_index):
        z = self.project(self.encoder(x, edge_index))
...
def prepare_lp_data(data, train_mask, val_mask, test_mask, device, num_classes, msg_split_ratio, neg_pos_ratio):
    ...
    #manually add rev_is
    train_data_LP['label', 'rev_is', 'paper'].edge_index = train_data_LP['paper', 'is', 'label'].edge_index.flip(0)
    val_data_LP['label', 'rev_is', 'paper'].edge_index = val_data_LP['paper', 'is', 'label'].edge_index.flip(0)
    test_data_LP['label', 'rev_is', 'paper'].edge_index = test_data_LP['paper', 'is', 'label'].edge_index.flip(0)
...
class LinkPredictor(torch.nn.Module):
    def __init__(self, hidden_channels, embedding_size, metadata, two_layers):
        ...
        self.pre = torch.nn.ModuleDict({
            nt: Linear(-1, hidden_channels, bias=True)
            for nt in metadata[0]
        })

        self.encoder = to_hetero(GIN(hidden_channels, two_layers), metadata)
    ...
    def forward(self, x_dict, edge_index_dict):
        x_dict = {nt: self.pre[nt](x) for nt, x in x_dict.items()}

        z_dict = self.encoder(x_dict, edge_index_dict)
        return {k: self.project(v) for k, v in z_dict.items()}
...
def main():
    ...
    for seed in seed_list:
        ...
        model_NC = GIN_NC(hidden_channels, embedding_size, num_classes ,two_layers).to(device)
