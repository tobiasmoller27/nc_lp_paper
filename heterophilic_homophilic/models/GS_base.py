...
class HeteroNCModel(torch.nn.Module):
    def __init__(self, hidden_channels, middle_size, embedding_size, num_classes, metadata, two_layers):
        super().__init__()
        self.hetero_encoder = to_hetero(
            GraphSAGE(hidden_channels, middle_size, embedding_size, two_layers),
            metadata
        )
